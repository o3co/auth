...
DSL 不要の ABAC ポリシーエンジン。HTTP サービス（`POST /verify`）として動作、またはライブラリとして組み込み可能。認可ロジックは TypeScript の Collector パターンで組み立てる。JWT 検証アルゴリズム設定可能 — HS256, RS256, ES256, EdDSA に対応、JWKS URI または公開鍵直接指定（auth.provider と対称設計）。OPA や Cedar にドロップイン置き換え可能 — `grpc.authz` は3つすべてをバックエンドとしてサポート。
